...
function detectCombos(cardIds) {
    ...
    if (toySets.every(ids => ids.some(has))) combos.push({ name: '四玩具', score: 3, cards: [] });
    ...
    // 新组合
    if (has('yukata') && has('uchiwa') && has('hanabi')) combos.push({ name: '夏祭', score: 4, cards: ['yukata', 'uchiwa', 'hanabi'] });
    ...
    if (sakura && has('m02_kemono') && has('kaze') && has('m08_mono')) combos.push({ name: '花鸟风月', score: 4, cards: [sakura, 'm02_kemono', 'kaze', 'm08_mono'] });
    if (sakura && has('kaze') && has('yuki') && has('m08_mono')) combos.push({ name: '风花雪月', score: 3, cards: [sakura, 'kaze', 'yuki', 'm08_mono'] });
    ...
    if (has('hi') && has('m08_mono') && has('yuki') && has('m05_mono_02')) combos.push({ name: '四天象', score: 3, cards: ['hi', 'm08_mono', 'yuki', 'm05_mono_02'] });
