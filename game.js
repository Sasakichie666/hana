...
const GameState = {
    ...
    selectedCardCombos: [],        // 当前选中手牌的理论组合列表
    phase: 'waiting',
    koikoiPending: false,
    isHost: false,
    peer: null,
...
function renderOpponents() {
    ...
    GameState.players.forEach(pid => {
        ...
        div.innerHTML = `<h3>对手: ${name}</h3>
            <div class="opponent-cards" id="oh-${pid}"></div>
            <div class="opponent-melded" id="om-${pid}"></div>`;
        container.appendChild(div);

        const handContainer = document.getElementById(`oh-${pid}`);
        const cnt = (GameState._oppHands[pid] || []).length;
        for (let i = 0; i < cnt; i++) {
            const b = document.createElement('div');
            b.className = 'opponent-card';
            b.textContent = '🪭';
            handContainer.appendChild(b);
        }

        const meldContainer = document.getElementById(`om-${pid}`);
        renderOpponentMelded(meldContainer, GameState._oppMelded[pid] || []);
    });
}
